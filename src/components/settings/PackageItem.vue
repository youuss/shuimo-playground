...
const isInstalled = computed(() =>
  orchestrator.packages.some(({ name }) => name === props.name)
)
...
const uninstall = () =>
  (orchestrator.packages = orchestrator.packages.filter(
  ...
  <div bg="dark:dark-500" border="~ rounded light-700 dark:dark-900" p="4">
    ...
    <div flex="~ row" m="t-2">
      ...
      <div
        flex="~"
        space="x-[1px]"
        bg="light-900 dark:dark-900"
        border="~ rounded dark:dark-900"
      >
        <!-- <PackageVersion v-model="version" v-if="source === 'skypack'" :name="name" /> -->
        <Button
          v-if="isInstalled"
          border="rounded-r"
          :class="{ rounded: source !== 'skypack' }"
          @click="uninstall()"
        >
          <carbon-delete />
          <span> Uninstall </span>
