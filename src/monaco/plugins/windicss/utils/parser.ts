...
export class HTMLParser {
  ...
  removeComments() {
    ...
    while ((match = regex.exec(this.html as string))) {
      if (match)
        this.html =
          (this.html as string).slice(0, match.index) +
          ' '.repeat(regex.lastIndex - match.index) +
          this.html.slice(regex.lastIndex)
    }
  }
  ...
  parseAttrs(): Attr[] {
    if (!this.html) return []
    const output: Attr[] = []
    const regex = /\S+\s*=\s*"[^"]+"|\S+\s*=\s*'[^']+'|\S+\s*=\s*[^\s>]+/gim
    let match
    while ((match = regex.exec(this.html as string))) {
      if (match) {
        const raw = match[0]
        ...
        let vstart =
          match.index + (sep + 1 + (raw.slice(sep + 1).match(/\S/)?.index || 0))
  ...
  parseClasses(): ClassName[] {
    ...
    if (!this.html) return []
    const classRegex =
      /\s(?:class|className|w:dark|w:light|w:active|w:after|w:before|w:checked|w:disabled|w:focus|w:hover|w:tw)=(["'])([^{}]+)\1/
    const quoteRegex = /["']/
    const classNames = []
    let _indexStart = 0
    ...
    while (propStart !== -1) {
      const afterMatch = _htmlLeft.slice(Math.max(0, propStart))
      const relativeStart = afterMatch.search(quoteRegex)
      const relativeEnd = afterMatch
        .slice(Math.max(0, relativeStart + 1))
        .search(quoteRegex)
      ...
      _htmlLeft = _htmlLeft.slice(Math.max(0, absoluteEnd + 2))
  ...
  parseApplies(): ClassName[] {
    ...
    const regex =
      /(?<=@apply\s+)[^;]+(?=\s+!important)|(?<=@apply\s+)[^;]+(?=;)/gim
    let match
    while ((match = regex.exec(this.html as string))) {
      if (match) {
        output.push({
