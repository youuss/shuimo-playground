...
const resolveVariants = (document: TextDocument, position: Position) => {
  ...
  if (
    (!pattern || text.match(pattern) === null) &&
    text.match(patterns.html) === null
  ) {
    const key = text.match(/\S+(?=\s*=\s*["']?[^"']*$)/)?.[0]
    if (!key || !isAttrVariant(key, variants)) return []
  }
...
const resolveAttrKeys = (document: TextDocument, position: Position) => {
  ...
  if (
    ...
    return [
      ...Object.keys(_completions.attr.static).map((label) =>
        attrKey(label, CompletionItemKind.Field, 0)
      ),

      ...Object.keys(variants).map((label) =>
        attrKey(label, CompletionItemKind.Module, 1)
      ),
    ]
  }
...
const resolveAttrValues = (document: TextDocument, position: Position) => {
  ...
  if (text.match(/(<\w+\s*)[^>]*$/) !== null) {
    const key = isAttrUtility(
      text.match(/\S+(?=\s*=\s*["']?[^"']*$)/)?.[0] as string,
      _completions.attr.static
    )
    ...
    completions.push(
      ..._completions.attr.static[key].map((label, index) => ({
        label,
        kind: CompletionItemKind.Constant,
        detail: key,
        sortText: `1-${index.toString().padStart(8, '0')}`,
    ...
    if (key in _completions.attr.color) {
      completions.push(
        ..._completions.attr.color[key].map(({ label, doc }, index) => ({
          label,
          kind: CompletionItemKind.Color,
          detail: key,
          sortText: `0-${index.toString().padStart(8, '0')}`,
    ...
    if (key in _completions.attr.bracket) {
      completions.push(
        ..._completions.attr.bracket[key].map((label, index) => ({
          label,
          kind: CompletionItemKind.Struct,
          detail: key,
          sortText: `3-${index.toString().padStart(8, '0')}`,
    ...
    if (key in _completions.attr.dynamic) {
      completions.push(
        ..._completions.attr.dynamic[key].map(({ label }, index) => ({
          label,
          kind: CompletionItemKind.Variable,
          detail: key,
          sortText: `4-${index.toString().padStart(8, '0')}`,
