...
export function isCursorInHTMLAttribute({
  html,
  document,
  position,
}: HTMLPluginCompletion) {
  ...
  if (currentNode.start && currentNode.endTagStart) {
    ...
    if (!currentNode.attributes) return false

    const attrs = Object.entries(currentNode.attributes).map(
      ([key, value]) => `${key}=${value}`
    )
    const indexes = attrs.flatMap((attr) => {
      return [...tag.matchAll(new RegExp(attr, 'gi'))].flatMap((x) => ({
        start: x.index,
